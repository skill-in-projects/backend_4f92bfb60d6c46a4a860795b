...
async function sendErrorToEndpoint(endpointUrl, boardId, req, error) {
    try {
        ...
        for (let i = 1; i < stackLines.length; i++) {
            ...
            // Pattern: match parentheses with file path (non-greedy), then :line:column
            ...
            // Second try: "at file:line:column" (no function name, no parentheses)
            fileMatch = line.match(/at\\s+([^\\s]+):(\\d+):(\\d+)/);
            if (fileMatch && fileMatch[1]) {
                const filePath = fileMatch[1].trim();
                // Extract just the filename
                const lastSlash = filePath.lastIndexOf('/');
